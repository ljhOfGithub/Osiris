# enable exceptions instead of ignoring them是否忽视报错
IGNORE_EXCEPTIONS = 0

# enable reporting of the result
REPORT_MODE = 0

#print everything in the console
PRINT_MODE = 0

# enable data flow analysis (incomplete yet)
DATA_FLOW = 1

# enable log file to print all exception
DEBUG_MODE = 0

# check false positive in concurrency检查假阳性
CHECK_CONCURRENCY_FP = 0

# Timeout for z3 (symbolic execution) in ms符号执行的时间上限
TIMEOUT = 100

# Timeout for z3 (arithmetic) in ms算术计算的时间上限
ARITHMETIC_TIMEOUT = 15000

# Set this flag to 1 if we want to do unit test from file unit_test.json
# Set this flag to 2 if we want to do evm real value unit test
# Set this flag to 3 if we want to do evm symbolic unit test
UNIT_TEST = 0

# timeout to run symbolic execution (in secs)
GLOBAL_TIMEOUT = 50

# timeout to run symbolic execution (in secs) for testing
GLOBAL_TIMEOUT_TEST = 2

# print path conditions
PRINT_PATHS = 0

# WEB = 1 means that we are using Oyente for web service
WEB = 0

# Redirect results to a json file.将结果重定向到json文件。
STORE_RESULT = 0

# depth limit for DFS
DEPTH_LIMIT = 50

GAS_LIMIT = 4000000

LOOP_LIMIT = 10

# Use a public blockchain to speed up the symbolic execution
#使用公开的区块链来加速符号执行
USE_GLOBAL_BLOCKCHAIN = 0

# Take state data from state.json to speed up the symbolic execution
INPUT_STATE = 0

# Check assertions
CHECK_ASSERTIONS = 0

GENERATE_TEST_CASES = 0

# CFG = 1 means that we create a control flow graph and store it as .dot file
# CFG = 1表示创建一个控制流图，并将其存储为.dot文件
CFG = 0

# MODEL_INPUT = 1 means that we output the models generated by the solver
MODEL_INPUT = 0

# Directory to store json result files
RESULTS_DIR = "results/"

# (Experimental) Interprocedural taint analysis (1) or Intraprocedural taint analysis (0)
# (实验)过程间污染分析(1)或过程内污染分析(0)
INTERPROCEDURAL_TAINT = 0
